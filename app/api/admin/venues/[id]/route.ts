...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (error || !venue) {
      return NextResponse.json({ error: error?.message ?? "No se pudo actualizar la sede" }, { status: 400 })
    }

    return NextResponse.json({ venue })
  } catch (e) {
    const message = e instanceof Error ? e.message : "Error interno"
    console.error("PATCH /api/admin/venues/[id] failed:", e)
...
export async function DELETE(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { error } = await auth.adminClient.from("venues").delete().eq("id", id)
    ...
    const message = e instanceof Error ? e.message : "Error interno"
    console.error("DELETE /api/admin/venues/[id] failed:", e)
    return NextResponse.json({ error: message }, { status: 500 })
  }
}
