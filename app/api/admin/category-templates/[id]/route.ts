...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: template, error } = await auth.adminClient
    ...
    console.error("PATCH /api/admin/category-templates/[id] failed:", e)
...
export async function DELETE(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { error } = await auth.adminClient.from("category_templates").delete().eq("id", id)
    if (error) {
    ...
    const message = e instanceof Error ? e.message : "Error interno"
    console.error("DELETE /api/admin/category-templates/[id] failed:", e)
    return NextResponse.json({ error: message }, { status: 500 })
  }
}
