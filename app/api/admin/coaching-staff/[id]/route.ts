...
const updateCoachingStaffSchema = z.object({
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = updateCoachingStaffSchema.safeParse(body)
    ...
    console.error("PATCH /api/admin/coaching-staff/[id] failed:", e)
...
export async function DELETE(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { error } = await auth.adminClient.from("coaching_staff").delete().eq("id", id)
    if (error) {
    ...
    const message = e instanceof Error ? e.message : "Error interno"
    console.error("DELETE /api/admin/coaching-staff/[id] failed:", e)
    return NextResponse.json({ error: message }, { status: 500 })
  }
}
